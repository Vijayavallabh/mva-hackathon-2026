...
## A falsifiable model-first decision, with no rescue-priority drug

Participant jvv7 · research draft 16 · 2026-09-24

Repository: https://github.com/Vijayavallabh/mva-hackathon-2026

Computational research; no wet-lab experiments have been performed. Drug efficacy and clinical exposure
...
The v15 audit adds a [19-claim falsification register](track2-falsification-register-v15.json)
and [reproducible adversarial review](track2-falsification-review-v15.md). Newer protein
models pass their narrow primary gate, but an expanded-control sensitivity analysis
fails separation in 8/12 model/window comparisons. The hypercapnia supplement has an
unresolved ex vivo unit discrepancy. These findings narrow the computational claim,
quarantine the disputed concentration and strengthen measurement and advancement rules.
No model or software check supplies the missing biological evidence.
...
RNA decay versus truncated protein has not been measured. Computational evidence for
the missense allele now includes newer models that pass the fixed control challenge,
alongside older model disagreement and failed experimental-control checks.
The earlier annotation support is not independent functional confirmation; instability,
dominant-negative activity and residual function have not been demonstrated.
[Hanks 2004](https://doi.org/10.1038/ng1449),
[Suijkerbuijk 2010](https://doi.org/10.1158/0008-5472.CAN-09-4319)
...
### What expanded protein modeling changes

The owner-hosted protein campaign completed seven ESM checkpoints, 48 Boltz-2
predictions and 120 AlphaFold2 predictions. Its fixed controls and all results are retained in
[the model comparison](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/track2-model-expansion.md). These are correlated computational
sensitivity analyses, not biological replications.

For N1002K, the five-checkpoint ESM-1v ensemble gives mean sequence log-score
changes of −0.055, −0.105 and −0.248 across the three fixed windows. Individual
checkpoints change sign in every window. Only two checkpoints pass the primary
experimental-control ordering in all windows; the full ensemble fails in one.
ESM-2 at 3B and 15B gives stronger negative candidate scores but fails controls
in two and three windows, respectively. Larger negative values cannot be promoted
to validated loss of function, and the earlier positive pilot cannot establish
benignity. These scores are not calibrated across model sizes.

Adding a shared public wild-type alignment makes Boltz structures more confident
and reproducible, but impaired controls also look well folded. AlphaFold2 likewise
produces confident, similar folds for both impaired controls and N1002K across all
five parameter sets and three seeds. The domain comparisons cannot measure
endogenous abundance, partner recruitment, checkpoint function or drug response.
They neither establish normal function nor identify a validated folding defect.
The two audited human experimental structures lack coordinates for this domain.

V13 withdrew v12's unqualified missense-support statement. The newer results below
pass a narrow primary control check but remain sensitive to the control set; earlier
failures remain relevant. Neither campaign overturns the
preserved ranked submission, classify either allele, resolve phase or change the
drug decisions. Endogenous RNA, protein abundance and localization, cell division
and tissue function remain necessary. Structural confidence cannot qualify a
folding-rescue intervention or substitute for those measurements.

### What the newer models change

The [newer-model audit](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/track2-latest-models.md)
adds ESMC 300M, 600M and 6B plus ESM3-open 1.4B: 84 protein scores across the same
three windows and six published engineered controls. All four models pass the small
prespecified primary control-ordering gate in all windows and score N1002K negatively.
This is a narrow control-ordering result. The v15 reanalysis retains that success,
but adding the already-retained secondary controls breaks complete separation in 8/12
model/window comparisons. Retained-function controls have negative scores in 11/24
comparisons and impaired controls positive scores in 2/48. Score sign therefore cannot
classify function. This post-hoc sensitivity analysis does not replace the original
gate, calibrate pathogenicity or independently replicate biology. One primary retained-function control and three impaired controls from one
study cannot establish general predictive accuracy.

| New model / N1002K log score | N-terminal 1–1022 | C-terminal 29–1050 | Domain 721–1044 |
| --- | ---: | ---: | ---: |
| ESMC 300M | −0.190749 | −0.262950 | −0.090961 |
| ESMC 600M | −2.309415 | −2.664999 | −1.859905 |
| ESMC 6B | −6.362635 | −5.645506 | −4.718565 |
| ESM3-open 1.4B | −0.269117 | −0.132473 | −0.075246 |

These are masked alternate-minus-reference log probabilities, not effect sizes
comparable across models. Exact repeated masked distributions agree; all controls
and windows are retained. ESM3 refers to the open 1.4B checkpoint, not hosted 98B.

The same follow-up adds 192 domain structures: 120 AlphaFold3, 24 ESM3 and 48
ESMFold2 predictions, with all seeds and both ESMFold2 alignment arms retained.
AlphaFold3 mean CA pLDDT ranges are 90.93–91.33 for WT, 91.12–91.59 for impaired
D911N and 90.83–91.19 for N1002K. ESMFold2 also confidently folds impaired controls.
ESM3 WT seed-pair RMSDs span 7.10–9.10 Å, so candidate differences cannot simply
be attributed to mutation. ESMFold2's frozen ESMC 6B backbone and the reused alignment
limit independence. These 324-residue domain predictions measure neither endogenous
protein function nor drug rescue. Seeds and pairwise comparisons are correlated
computations; they are not biological replicates or clinical confidence intervals.

**AlphaFold3-derived summaries and interpretation are modifications of model output,
provided under the [AlphaFold3 Output Terms](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/alphafold3-output-terms.md)
and accompanying [Legally Binding Terms of Use notice](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/alphafold3-Legally-Binding-Terms-of-Use.txt).**
No coordinates or parameters were modified or redistributed in this report.
[Abramson et al., Nature 2024](https://doi.org/10.1038/s41586-024-07487-w).

### What Evo2 adds

Evo2 7B, 20B and 40B passed fixed numerical checks and the same 96-variant public BRCA1
functional benchmark (descriptive AUROCs 0.942, 0.920 and 0.920). The 20B follow-up
completes all 100 comparisons and exactly matches its official mean-loss and repeat
checks. Equal AUROCs do not imply identical predictions; this is one reused benchmark. Both report-derived
substitutions had negative alternate-minus-reference DNA log scores in both fixed
windows; all strand-specific outputs are retained.

| Model / candidate | 8,192 bases | 32,768 bases |
| --- | ---: | ---: |
| Evo2 7B / L737Ter | −0.007684 | −0.002035 |
| Evo2 7B / N1002K | −0.001611 | −0.000343 |
| Evo2 20B / L737Ter | −0.010769 | −0.002713 |
| Evo2 20B / N1002K | −0.002082 | −0.000538 |
| Evo2 40B / L737Ter | −0.010580 | −0.002736 |
| Evo2 40B / N1002K | −0.002087 | −0.000533 |

These are context-dependent whole-window mean log-score changes averaged across
strands, not clinical probabilities. Longer-window averaging changes their scale.
BRCA1 performance does not establish BUB1B accuracy; pretraining overlap and
cross-gene transfer remain uncertain. Public-reference windows and synthetic
alternates are not subject haplotypes. Agreement does not establish NMD, splicing,
phase or drug response, and does not resolve allele function or the older protein-model
control limitations. The newer protein gate successes remain separate evidence.

The 40B FP8 runtime needed an isolated library correction. Its two-H100 run passed
the benchmark, then exhausted memory on a long candidate window. All four candidate
comparisons subsequently completed on all eight H100s, repeating numerical checks
and exactly matching reference and duplicated short-window scores. Failed attempts,
partial results and the hardware split remain explicit. A memory failure is not
biological evidence, and these computations are not wet-lab results.
[Full results, fixed plans and runtime audit](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/track2-model-expansion.md).
...
The [v15 decision ledger](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/track2-evidence-v15.json)
supports this report without replacing the historical ledgers. Its 63 cumulative source records
...
numbers of validated studies. That cycle produced the historical v10 ledger with 52 source records and
11 candidate decisions, including rechecked registry/baseline sources. The current
v15 ledger adds 11 source records and retains those 11 drug dispositions.
...
mTOR readouts. Key experiments were small and some displayed controls were reused.
The v15 review now checks selected supplemental Methods: PDF page 12 reports 10 mM
ex vivo rapamycin, whereas page 15 reports 10 μM for primary cells. Visual inspection
confirms the 1,000-fold reporting discrepancy across related ex vivo methods. Neither
value is silently corrected or adopted as a design concentration. The qualitative
finding is retained, while the exposure-linked inference requires clarification.
[Supplement](https://insight.jci.org/articles/view/182842/sd/pdf/render/1). This is a reason to
...
### What the full-chain falsification audit changes

The [v15 review](track2-falsification-review-v15.md) and
[analysis](track2-falsification-analysis-v15.json) add five operational safeguards:

- **Endogenous mechanism:** a stop annotation is not a measured null. Published 731X
  complementation and later domain-removal experiments have different endpoints and
  expression contexts. Test RNA decay, residual protein, attachment, checkpoint
  activation/maintenance and silencing separately; the selected L737Ter is not 731X.
  [2012](https://doi.org/10.1016/j.devcel.2012.03.009),
  [2020](https://doi.org/10.1016/j.celrep.2020.108397).
- **Valid measurement:** useful output per enrolled culture must accompany survivor
  fractions. Direct counts and growth-rate-aware sensitivity checks challenge tumour
  selectivity driven by proliferation differences. Growth-rate normalization is not
  valid for non-growing controls or a substitute for mature-lineage function.
  [Hafner](https://doi.org/10.1038/nmeth.3853).
- **Assay artifacts and mediation:** flux needs pH/expression/interference controls and
  orthogonal degradation evidence. Pathway suppression needs functional follow-up and
  feedback-aware, time-matched measurements; it does not license a new combination.
  [Flux reporter study](https://pmc.ncbi.nlm.nih.gov/articles/PMC3121655/),
  [Feedback](https://doi.org/10.1158/0008-5472.CAN-05-2925).
- **Symmetric safety review:** retain favorable pediatric TSC follow-up and a specific
  pediatric transplant regimen's safety result as counterweights to universal-harm
  claims. Different indication, comparator, regimen and timing prevent transfer to MVA.
  The 2026 BIOMEDE abstract likewise reports both futility against a historical DIPG
  control and favorable tolerability/exploratory signals; it does not establish RMS
  efficacy or a general biomarker rule. These newer trials were read at abstract level.
  [Transplant trial](https://doi.org/10.1001/jama.2025.14338),
  [BIOMEDE](https://doi.org/10.1038/s41591-026-04354-1).
- **Decision discipline:** an unmeasured requirement or imprecise null is unresolved;
  an adequately informative negative test rejects only the tested claim/context.
  Passing safety requires prespecified bounds, not a nonsignificant toxicity test.
  Model-only evidence cannot satisfy a biological advancement gate. All current gates
  remain on hold; all-pass software could allow only further preclinical review.

The 2026 TPR-domain work concerns full checkpoint interactions omitted by the modeled
C-terminal fragment. A two-family report about other heterozygous variants retains VUS
classifications and provides no selected-pair or family-genotype evidence. Preserve
single-allele/cis alternatives and the separate family-history dimension.
[TPR](https://doi.org/10.1038/s42003-026-10384-9),
...
The [v15 validation plan](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/track2-validation-v15.md)
...
September 19. That review found the running public app and Space metadata at the established
public revision; this cycle did not repeat authenticated portal checks. The official task accepts mechanism-grounded hypotheses for further
...
This v15 report preserves both owner-hosted model campaigns and adds an adversarial
reanalysis of the retained scores. The v15 ledger and protocol carry the new source
and assay decisions while preserving the v10 drug dispositions. Every previous
release, including v14 and its bound inputs, is preserved. The [earlier campaign](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/track2-model-expansion.md)
and [newer comparison](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/track2-latest-models.md) retain fixed plans, complete
results, failed attempts and runtime corrections. Local checks establish provenance
and declared invariants, not clinical validity.

The [nine-page v15 slides](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/track2-slides-v16.html), [aligned read-aloud transcript](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/track2-pitch-v16.md)
and [video description](https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/track2-video-description-v16.md) integrate these findings
and AlphaFold3 disclosure. Include the full acknowledgement page inside the
three-minute video and retain the output terms/notice with distributed derivatives.
Recording, hosting, owner review, live authenticated checks and a receipt remain open.
No upload occurred here. The current artifact record and startup harness check keep
future sessions on the same revision; historical instructions are explicitly archived.

The portal requires a PDF or Markdown report with participant/team identity in its
filename, a GitHub URL and the video link. Use a newly reviewed participant-named report; the preserved v14 package does not contain the new falsification findings.
...
editorial work and historical independent agent reviews. The v15 cycle is an author
audit; no independent agent or specialist review was performed in this cycle. The owner attests that content is not used
...
The expanded modeling used Meta ESM-1v/ESM-2, Boltz-2, AlphaFold2 through ColabFold
and Arc Institute Evo2 on the owner-authorized H100 host. Model packages, public weights and input/output
hashes are recorded. Only public protein/reference sequences, published controls
and permitted report-derived substitutions entered this workflow. One external
ColabFold alignment request received only the public wild-type BUBR1 domain;
training/retention for that service is not asserted. Neural inference on the owner's
host and external alignment search are distinct services. Local rendering adds no
provider. The model-comparison addendum must accompany future disclosure.

The follow-up used owner-hosted Biohub ESMC 300M/600M/6B, ESM3-open 1.4B,
ESMFold2, Google DeepMind AlphaFold3 and Evo2 20B. The existing public-WT alignment
was reused; no new hosted inference or alignment request was made. Fixed source,
weight and runtime hashes are recorded in the newer-model provenance. Preserve its
corrections: the initial GPU-filter omission, failed CCD-cache invocation, inherited
plan wording and the Evo environment-reconciliation flag. These are not hidden zero
scores or relaxed gates. No subject reads, source VCF records or clinical narrative
were transferred. AlphaFold3 findings are governed by the terms and modification
notice above. V14 integration uses local rendering and author verification, not a
new independent scientific review or additional biological inference.
...
output-term qualifications and historical scope, plus owner-hosted biomolecular
models and the external public-WT ColabFold alignment search. Unknown settings remain a final
...
conversions, figures and release hashes. Expanded sequence and structure models are
checked against published controls; model disagreement and failed qualification
remain in the evidence rather than selecting only favorable outputs. Newer ESMC/ESM3
primary-control success is qualified by the post-hoc expanded-control failures
(8/12 model/window comparisons) and negative retained-function scores (11/24).
Earlier failures remain visible; none of these checks qualifies a functional or
drug-rescue claim.
...
reviews, website review and v15 falsification audit are not new independent panels
or clinician reviews. The v15 audit records all nineteen claims, reading limits,
rejected bibliographic matches and unresolved concentration units.
...
not drug response or phase. Public UniProt, PDB and upstream model resources supply
protein sequences, coordinate audits and modeling inputs; the expanded campaign
retains its public experimental-control and reference-source provenance. The source ledger, exposure ledger and individual review
...
retrieval failures, model identity and contradictory evidence. Newer ESMC/ESM3
models pass their small primary control challenge, but expanded controls fail
separation in 8/12 model/window comparisons; 11/24 retained-function scores are
negative. This post-hoc sensitivity limits interpretation of candidate scores.
Earlier failures remain visible; confident structures do not validate allele function.
No new biological experiment is reported.
...
regenerative deficits; neither is established for the selected pair. A 1,000-fold
unit discrepancy in a supporting supplement leaves its concentration quarantined.
...
Clinical exposure margins remain unknown. Nineteen claim-specific falsifiers and
typed gates distinguish unknown/hold from demonstrated failure/stop; all-pass can
permit only preclinical review. Growth-rate, flux-reporter and missingness controls
test whether apparent benefit is a measurement artifact.
