...
Participant jvv7 · research draft 5 · 2026-09-19
...
We propose testing whether a drug can improve function in BUB1B-deficient non-cancer
cells, or suppress a relevant tumour, without worsening the underlying vulnerability
of normal tissues. These objectives need separate success criteria.

Everolimus is the sole conditional research priority for non-cancer function. The first
requirement is to establish excess mTORC1 activity in a qualified model. Only then would
we test whether changing that activity improves function at a justified exposure without
injury. This priority does not predict the best treatment. Hydroxychloroquine (HCQ)
remains a reserve candidate. Pralatrexate is an optional tumour-only direction for
fusion-positive rhabdomyosarcoma (RMS), with no claim that it repairs BUBR1 function.

Every tumour experiment must include a matched BUB1B-deficient non-cancer model and
track what happens to all cells. Killing abnormal cells, preventing mitotic entry or
allowing escape from arrest can improve a statistic calculated among survivors while
reducing useful division. Tumour potency and protein abundance alone would miss this.

The reviewed literature supports better controls and rejects unjustified exposure
comparisons, but does not promote another drug. Positive findings and contrary evidence
both inform the decisions below. The next decisive evidence requires experiments;
agreement between language models cannot supply replication. Corrected primary-source
reviews, rather than uncorrected GLM proposals, support this report.
...
The v5 release binds the report, unchanged scientific decision ledger, validation plan,
pitch, deck and supporting reviews to hashes. Track 1 v4 and earlier Track 2 snapshots,
including v2, v3 and v4, are preserved. Hashes detect drift; they do not establish truth,
signatures, clinical readiness or a portal receipt.
