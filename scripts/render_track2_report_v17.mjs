#!/usr/bin/env node
// Local-only preview of the prepared public-safe v17 report. No project env or subject inputs.
...
if (process.argv.length !== 3 || !/^[a-z][a-z0-9-]+$/.test(outputName ?? ''))
  throw new Error('Usage: node scripts/render_track2_report_v17.mjs EXISTING-DOCUMENT-OUTPUT-NAME');
const output = path.join(root, 'results/feat009', outputName);
const source = path.join(output, 'jvv7_track2_report_v17.html');
...
if (documents.files['jvv7_track2_report_v17.html'] !== hash(sourceBytes) ||
    documents.report_sha256 !== hash(await readFile(path.join(root,'notes/track2-report-v17.md'))))
...
try {
  ...
  const pdf = await page('Page.printToPDF', { printBackground: true, preferCSSPageSize: true,
    ...
    footerTemplate: '<div style="width:100%;font-family:Arial;font-size:8px;color:#655b70;padding:0 48px;display:flex;justify-content:space-between"><span>jvv7 · Track 2 v17 · Research hypothesis</span><span><span class="pageNumber"></span> / <span class="totalPages"></span></span></div>'  });
  const pdfBytes = Buffer.from(pdf.data, 'base64');
  await writeFile(path.join(output, 'jvv7_track2_report_v17.pdf'), pdfBytes, { flag: 'wx' });
  files['jvv7_track2_report_v17.pdf'] = hash(pdfBytes);
  ...
  const manifest = {source:'notes/track2-report-v17.md', source_sha256:documents.report_sha256,
  ...
  console.log(JSON.stringify({output,pdf:'jvv7_track2_report_v17.pdf',geometry,source_sha256:documents.report_sha256}));
