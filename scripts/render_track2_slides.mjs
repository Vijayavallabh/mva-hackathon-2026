#!/usr/bin/env node
// Local-only preview of the fixed public-safe v4 deck. No project env or subject inputs.
...
const profile = await mkdtemp(path.join(tmpdir(), 'mva-v4-render-'));
...
try {
  ...
  const navigation = await page('Page.navigate', { url: pathToFileURL(source).href });
  if (navigation.errorText) throw new Error('Local deck navigation failed');
  let complete = false;
  for (let attempt = 0; attempt < 50; attempt++) {
    const check = await page('Runtime.evaluate', { expression: "document.readyState === 'complete' && document.querySelectorAll('section.slide').length === 5", returnByValue: true });
    if (check.result.value === true) { complete = true; break; }
    await delay(100);
  }
  if (!complete) throw new Error('Deck did not load five slides');
  const geometry = await page('Runtime.evaluate', {
    expression: `Array.from(document.querySelectorAll('section.slide')).map(section => {
      ...
      return {id:section.id,x:rect.x + scrollX,y:rect.y + scrollY,width:rect.width,height:rect.height,outside};
    })`, returnByValue: true,
  });
  ...
  for (const [index, slide] of slides.entries()) {
    if (slide.id !== `slide-${index + 1}` || slide.width !== 1280 || slide.height !== 720 || slide.outside.length)
      throw new Error(`Unexpected layout or text outside slide ${index + 1}`);
  ...
  const manifest = { source: 'notes/track2-slides-v4.html', source_sha256: hash(sourceBytes),
    renderer_sha256: hash(rendererBytes),
    browser: await call('Browser.getVersion'), slides, files,
  ...
  console.log(JSON.stringify({ output, slides: slides.length, text_outside_slides: 0, source_sha256: manifest.source_sha256 }));
} finally {
