#!/usr/bin/env node
// Local-only preview of the fixed public-safe v10 deck. No project env or subject inputs.
...
const source = path.join(root, 'notes/track2-slides-v10.html');
...
if (process.argv.length !== 3 || !/^[a-z][a-z0-9-]+$/.test(outputName ?? '')) {
  throw new Error('Usage: node scripts/render_track2_slides_v10.mjs NEW-OUTPUT-NAME');
}
...
for (const target of [root, path.join(root, 'results'), outputRoot,
  ...
  path.join(root, 'scripts/track2_release_v10.py')]) {
...
const validation = spawnSync('/home/sports/.local/bin/uv',
  ['run', '--offline', 'python', 'scripts/track2_release_v10.py', 'check-deck'],
...
const profile = await mkdtemp(path.join(tmpdir(), 'mva-v10-render-'));
...
try {
  ...
  for (const [index, slide] of slides.entries()) {
    if (slide.id !== `slide-${index + 1}` || slide.width !== 1280 || slide.height !== 720 || slide.outside.length || slide.overlaps.length || slide.min_font_px < 24 || slide.svg_figures < 1)
  ...
  await writeFile(path.join(output, 'track2-slides-v10.pdf'), pdfBytes, { flag: 'wx' });
  files['track2-slides-v10.pdf'] = hash(pdfBytes);
  ...
  const manifest = { source: 'notes/track2-slides-v10.html', source_sha256: hash(sourceBytes),
