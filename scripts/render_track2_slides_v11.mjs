#!/usr/bin/env node
// Local-only preview of the fixed public-safe v11 deck. No project env or subject inputs.
...
const source = path.join(root, 'notes/track2-slides-v11.html');
...
if (process.argv.length !== 3 || !/^[a-z][a-z0-9-]+$/.test(outputName ?? '')) {
  throw new Error('Usage: node scripts/render_track2_slides_v11.mjs NEW-OUTPUT-NAME');
}
...
for (const target of [root, path.join(root, 'results'), outputRoot,
  ...
  path.join(root, 'scripts/track2_release_v11.py')]) {
...
const validation = spawnSync('/home/sports/.local/bin/uv',
  ['run', '--offline', 'python', 'scripts/track2_release_v11.py', 'check-deck'],
...
const profile = await mkdtemp(path.join(tmpdir(), 'mva-v11-render-'));
...
try {
  ...
  for (let attempt = 0; attempt < 50; attempt++) {
    const check = await page('Runtime.evaluate', { expression: "document.readyState === 'complete' && document.querySelectorAll('section.slide').length === 6", returnByValue: true });
    if (check.result.value === true) { complete = true; break; }
    await delay(100);
  }
  if (!complete) throw new Error('Deck did not load six slides');
  ...
  await writeFile(path.join(output, 'track2-slides-v11.pdf'), pdfBytes, { flag: 'wx' });
  files['track2-slides-v11.pdf'] = hash(pdfBytes);
  ...
  const manifest = { source: 'notes/track2-slides-v11.html', source_sha256: hash(sourceBytes),
