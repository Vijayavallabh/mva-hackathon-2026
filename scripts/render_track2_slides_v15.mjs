#!/usr/bin/env node
// Local-only preview of the fixed public-safe v15 deck. No project env or subject inputs.
...
const source = path.join(root, 'notes/track2-slides-v15.html');
...
if (process.argv.length !== 3 || !/^[a-z][a-z0-9-]+$/.test(outputName ?? '')) {
  throw new Error('Usage: node scripts/render_track2_slides_v15.mjs NEW-OUTPUT-NAME');
}
...
for (const target of [root, path.join(root, 'results'), outputRoot,
  ...
  path.join(root, 'scripts/track2_release_v15.py')]) {
...
const validation = spawnSync('/home/sports/.local/bin/uv',
  ['run', '--offline', 'python', 'scripts/track2_release_v15.py', 'check-deck'],
...
const profile = await mkdtemp(path.join(tmpdir(), 'mva-v15-render-'));
...
try {
  ...
  await writeFile(path.join(output, 'track2-slides-v15.pdf'), pdfBytes, { flag: 'wx' });
  files['track2-slides-v15.pdf'] = hash(pdfBytes);
  ...
  const manifest = { source: 'notes/track2-slides-v15.html', source_sha256: hash(sourceBytes),
