#!/usr/bin/env node
// Local-only preview of the fixed public-safe v16 deck. No project env or subject inputs.
...
const source = path.join(root, 'notes/track2-slides-v16.html');
...
if (process.argv.length !== 3 || !/^[a-z][a-z0-9-]+$/.test(outputName ?? '')) {
  throw new Error('Usage: node scripts/render_track2_slides_v16.mjs NEW-OUTPUT-NAME');
}
...
for (const target of [root, path.join(root, 'results'), outputRoot,
  ...
  path.join(root, 'scripts/track2_release_v16.py')]) {
...
const validation = spawnSync('/home/sports/.local/bin/uv',
  ['run', '--offline', 'python', 'scripts/track2_release_v16.py', 'check-deck'],
...
const profile = await mkdtemp(path.join(tmpdir(), 'mva-v16-render-'));
...
try {
  ...
  for (let attempt = 0; attempt < 50; attempt++) {
    const check = await page('Runtime.evaluate', { expression: "document.readyState === 'complete' && document.querySelectorAll('section.slide').length === 9", returnByValue: true });
    if (check.result.value === true) { complete = true; break; }
    await delay(100);
  }
  if (!complete) throw new Error('Deck did not load nine slides');
  ...
  for (const [index, slide] of slides.entries()) {
    if (slide.id !== `slide-${index + 1}` || slide.width !== 1280 || slide.height !== 720 || slide.outside.length || slide.overlaps.length || slide.min_font_px < 24 || slide.svg_figures !== (index < 8 ? 1 : 0))
  ...
  await writeFile(path.join(output, 'track2-slides-v16.pdf'), pdfBytes, { flag: 'wx' });
  files['track2-slides-v16.pdf'] = hash(pdfBytes);
  // AF3-derived slides travel with the exact terms and mandatory modification notice.
  for (const [input, name] of [
    ['notes/alphafold3-output-terms.md', 'AlphaFold3-Output-Terms.md'],
    ['notes/alphafold3-Legally-Binding-Terms-of-Use.txt', 'Legally-Binding-Terms-of-Use.txt']]) {
  ...
  const manifest = { source: 'notes/track2-slides-v16.html', source_sha256: hash(sourceBytes),
