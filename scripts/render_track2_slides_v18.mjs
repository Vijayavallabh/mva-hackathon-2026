#!/usr/bin/env node
// Local-only preview of the fixed public-safe v18 deck. No project env or subject inputs.
...
const source = path.join(root, 'notes/track2-slides-v18.html');
...
if (process.argv.length !== 3 || !/^[a-z][a-z0-9-]+$/.test(outputName ?? '')) {
  throw new Error('Usage: node scripts/render_track2_slides_v18.mjs NEW-OUTPUT-NAME');
}
...
const validation = spawnSync('/home/sports/.local/bin/uv',
  ['run', '--offline', '--no-project', 'python', '-c', 'import sys,json;sys.path.insert(0,"scripts");import track2_public_review_v18 as r;print(json.dumps(r.check_deck()))'],
...
try {
  ...
  await writeFile(path.join(output, 'track2-slides-v18.pdf'), pdfBytes, { flag: 'wx' });
  files['track2-slides-v18.pdf'] = hash(pdfBytes);
  ...
  const manifest = { source: 'notes/track2-slides-v18.html', source_sha256: hash(sourceBytes),
