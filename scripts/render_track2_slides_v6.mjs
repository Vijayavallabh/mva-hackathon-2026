#!/usr/bin/env node
// Local-only preview of the fixed public-safe v6 deck. No project env or subject inputs.
...
const source = path.join(root, 'notes/track2-slides-v6.html');
...
if (process.argv.length !== 3 || !/^[a-z][a-z0-9-]+$/.test(outputName ?? '')) {
  throw new Error('Usage: node scripts/render_track2_slides_v6.mjs NEW-OUTPUT-NAME');
}
...
for (const target of [root, path.join(root, 'results'), outputRoot,
  ...
  path.join(root, 'scripts/track2_release_v6.py')]) {
...
const validation = spawnSync('/home/sports/.local/bin/uv',
  ['run', '--offline', 'python', 'scripts/track2_release_v6.py', 'check-deck'],
...
const profile = await mkdtemp(path.join(tmpdir(), 'mva-v6-render-'));
...
try {
  ...
  await writeFile(path.join(output, 'track2-slides-v6.pdf'), pdfBytes, { flag: 'wx' });
  files['track2-slides-v6.pdf'] = hash(pdfBytes);
  ...
  const manifest = { source: 'notes/track2-slides-v6.html', source_sha256: hash(sourceBytes),
