...
class PublicReviewTests(unittest.TestCase):
    def setUp(self):
        self.report=(review.ROOT/'notes/track2-report-v18.md').read_text()
        self.deck=(review.ROOT/'notes/track2-slides-v18.html').read_text()
        self.requirements=json.loads((review.ROOT/'notes/track2-requirements-v18.json').read_text())
        self.community=json.loads((review.ROOT/'notes/track2-community-audit-v18.json').read_text())

    def test_current_public_review_and_methods(self):
        result=review.check()
        self.assertEqual((result['slides'],result['narration_words'],result['methods_fields']),(8,334,11))
