"""Preservation and content-boundary checks using the public v10 deck fixture."""
import hashlib
import json
...
import track2_release_v10 as release

DECK = (release.ROOT / 'notes/track2-slides-v10.html').read_text()
PUBLIC = {name: (release.ROOT / name).read_text() for name in (
    'notes/track2-report-v10.md', 'notes/track2-report-v6.md',
    'notes/track2-evidence-v10.json', 'notes/track2-evidence-v4.json')}
...
class ScientificVisualSnapshotTests(unittest.TestCase):
    def setUp(self):
        ...
        self.deck = self.root / 'notes/track2-slides-v10.html'
        self.deck.write_text(DECK)
        (self.root / 'results/feat009').mkdir(parents=True)
        self.output = self.root / 'results/feat009/visual_v10'
        for target in ('track2_release_v10.ROOT', 'track2_release_v6.ROOT'):
            mock = patch(target, self.root)
            mock.start()
            self.addCleanup(mock.stop)
        mock = patch('track2_release_v10.history', return_value='a' * 64)
    ...
    def test_build_preserves_inputs_and_v10_identity(self):
        ...
        self.assertEqual(json.loads(self.manifest().read_text())['schema_version'], 10)
    ...
    def test_changed_copy_rejected_even_with_forged_file_hash(self):
        release.build(self.output)
        name = 'jvv7_track2_report_v10.md'
        p = self.output / name
    ...
    def test_readiness_history_and_input_drift_are_rejected(self):
        ...
        (self.root / 'notes/track2-falsification-cycle1.md').write_text('Drift')
        with self.assertRaises(ValueError):
            release.verify(self.output)
    ...
    def test_adverse_and_favourable_evidence_must_both_survive(self):
        p = self.root / 'notes/track2-evidence-v10.json'
    ...
    def test_required_acknowledgement_preserved(self):
        p = self.root / 'notes/track2-report-v10.md'
