#!/usr/bin/env python3
"""Synthetic-only v5 release tests; no subject, credential or real package reads."""
...
import track2_release_v5 as release
from test_track2_release_v4 import evidence_fixture

REPORT = ('Research draft 5. Trans phase remains unconfirmed. Fireworks GLM. Google DeepMind. '
...
DECK = ('<!doctype html><html lang="en"><head><meta charset="utf-8">'
        ...
        '</head><body><main>' + ''.join(
            f'<section id="slide-{i}"><h1>Research only</h1><p>Phase unconfirmed; no clinical exposure margin.</p>{SVG}</section>'
...
class VectorPolicyTests(unittest.TestCase):
    ...
    def test_valid_static_vector_deck(self):
        self.assertEqual(self.check()['conceptual_vector_figures'], 5)
...
class ReleaseFixture(unittest.TestCase):
    def setUp(self):
        temporary = tempfile.TemporaryDirectory(prefix='mva-v5-test-')
        ...
        (self.root / 'notes/track2-report-v5.md').write_text(REPORT)
        (self.root / 'notes/track2-pitch-v5.md').write_text(PITCH)
        (self.root / 'notes/track2-slides-v5.html').write_text(DECK)
        ...
        self.out = self.root / 'results/feat009/synthetic-v5'
...
class BuildTests(ReleaseFixture):
    def test_fixed_deck_check_returns_exact_validated_hash(self):
        ...
        self.assertEqual(result['conceptual_vector_figures'], 5)
    ...
    def test_fixed_deck_check_rejects_active_payload_before_browser(self):
        path = self.root / 'notes/track2-slides-v5.html'
    ...
    def test_build_verify_preserves_sources_and_history(self):
        ...
        self.assertEqual(set(self.manifest()['historical_manifest_hashes']), {'v2', 'v3', 'v4'})
        self.assertFalse(result['upload_ready'])
        self.assertTrue(result['historical_v4_preserved'])
    ...
    def test_path_scope_and_history_immutable(self):
        ...
        cases += [self.root / f'results/feat009/jvv7_track2_research_v{i}' for i in range(1, 5)]
        for path in cases:
    ...
    def test_source_symlink_or_missing_input_rejected_before_writes(self):
        source = self.root / 'notes/track2-v5-design.md'
    ...
    def test_history_symlink_rejected_before_prior_verifier(self):
        ...
        path.unlink()
        path.symlink_to(self.root / 'notes/track2-report-v5.md')
        with self.assertRaises(ValueError):
            self.build()
        self.prior_verify.assert_not_called()
    ...
    def test_midbuild_input_drift_does_not_produce_manifest(self):
        ...
        def changed_copy(source, target):
            ...
            (self.root / 'scripts/test_track2_release_v5.py').write_text('synthetic drift')
...
class VerifyTests(ReleaseFixture):
    ...
    def test_truthy_and_coercible_readiness_claims_rejected(self):
        original = self.manifest()
        for field, bad in (('schema_version', 5.0), ('upload_ready', 0), ('upload_ready', True),
    ...
    def test_current_bound_source_drift_rejected(self):
        (self.root / 'scripts/render_track2_slides_v5.mjs').write_text('drift')
        with self.assertRaises(ValueError):
            release.verify(self.out)
    ...
    def test_historical_manifest_forgery_rejected(self):
        value = self.manifest()
        value['historical_manifest_hashes']['v4'] = 'a' * 64
        write_json(self.out / 'manifest.json', value)
        with self.assertRaises(ValueError):
            release.verify(self.out)
