...
import zipfile
import track2_release_v17 as release

ROOT=release.ROOT
SLIDES=ROOT/'results/feat009/v17-slide-review-release-20260924'
DOCS=ROOT/'results/feat009/v17-documents-final-20260924'
DEST=ROOT/'results/feat009/jvv7_track2_video_materials_v17.zip'


def sources():
    release.scientific_checks()
    render=json.loads((ROOT/'notes/track2-v17-render-audit.json').read_text())
    docs=json.loads((ROOT/'notes/track2-v17-documents-audit.json').read_text())
    ...
    files['jvv7_track2_slides_v17.html']=ROOT/'notes/track2-slides-v17.html'
    for name in ['pitch','transcript','video-description','reviewer-guide','owner-readiness']:
        suffix='txt' if name=='transcript' else 'md'
        files[f'jvv7_track2_{name}_v17.{suffix}']=ROOT/f'notes/track2-{name}-v17.{suffix}'
    return files
