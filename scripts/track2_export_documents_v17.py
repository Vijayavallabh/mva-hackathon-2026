...
# ///
"""Export the v17 report HTML and a filled copy of the pinned public methods template."""
import argparse
...
CSS='''@page{size:A4;margin:18mm 17mm 18mm}body{font-family:"Nimbus Sans",sans-serif;color:#241d32;font-size:11pt;line-height:1.4;margin:0}h1{font-family:"Liberation Serif",serif;font-weight:400;font-size:28pt;line-height:1.12;color:#30203e;margin:0 0 18pt}h2{font-family:"Liberation Serif",serif;font-weight:400;font-size:19pt;line-height:1.2;color:#634285;margin:20pt 0 9pt;break-after:avoid}h3{font-size:13pt;break-after:avoid}p{margin:0 0 10pt;orphans:3;widows:3}a{color:#17666c;text-decoration:underline;overflow-wrap:anywhere}strong{font-weight:700}table{border-collapse:collapse;width:100%;font-size:9.6pt;line-height:1.4;margin:12pt 0 16pt;table-layout:fixed}thead{display:table-header-group}tr{break-inside:avoid}th{background:#eae3f2;color:#30203e;text-align:left}td,th{border-bottom:1px solid #ddd9e3;padding:7pt 7pt;vertical-align:top;overflow-wrap:anywhere}th:nth-child(1){width:30%}pre{white-space:pre-wrap;overflow-wrap:anywhere;background:#f1edf6;padding:10pt;font-size:9.5pt;line-height:1.45;break-inside:avoid}code{font-family:"Nimbus Mono PS",monospace;font-size:.93em}li{margin-bottom:5pt}hr{border:0;border-top:1px solid #ddd9e3}*{-webkit-print-color-adjust:exact;print-color-adjust:exact}'''
...
def main():
    ...
    report=(ROOT/'notes/track2-report-v17.md').read_text();answers=review.report_checks(report)
    ...
    out.mkdir()
    name='jvv7_track2_methods_v17.xlsx';wb.save(out/name)
    ...
    page='<!doctype html>\n<html lang="en"><head><meta charset="utf-8"><meta http-equiv="Content-Security-Policy" content="default-src \'none\'; style-src \'unsafe-inline\'; base-uri \'none\'; form-action \'none\'"><title>jvv7 · Track 2 report v17</title><style>'+CSS+'</style></head><body><main>'+body+'</main></body></html>\n'
    if re.search(r'<(?:script|iframe|img|object|embed)\b',page,re.I):raise ValueError('Active or remote report element')
    (out/'jvv7_track2_report_v17.html').write_text(page)
    (out/'jvv7_track2_report_v17.md').write_text(report)
