...
def presentation_checks(deck):
    ...
    require('https://github.com/Vijayavallabh/mva-hackathon-2026/blob/main/notes/track2-report-v18.md' in c.links,'Current report link required')
...
def check_deck():
    p=ROOT/'notes/track2-slides-v18.html'
...
def check():
    deck=check_deck()
    r=json.loads((ROOT/'notes/track2-requirements-v18.json').read_text())
    community=json.loads((ROOT/'notes/track2-community-audit-v18.json').read_text())
    requirements=requirements_checks(r,community)
    methods=report_checks((ROOT/'notes/track2-report-v18.md').read_text())
    ...
    pitch=(ROOT/'notes/track2-pitch-v18.md').read_text()
    ...
    require(sum(counts)==334 and 'Narration contains 334 whitespace-separated words' in pitch,'Narration count drift')
    ...
    transcript='Track 2 v18 — read-aloud transcript\nRead the paragraphs; headings and times are cues, not narration.\n\n'
    for head,body in rows:transcript+='Slide '+head.replace(' / ',' | ')+'\n'+body.strip()+'\n\n'
    require((ROOT/'notes/track2-transcript-v18.txt').read_text()==transcript.rstrip()+'\n','Plain transcript drift')
    desc=(ROOT/'notes/track2-video-description-v18.md').read_text()
    ...
    return dict(passed=True,presentation_version=18,drug_science_version=15,**deck,narration_words=sum(counts),words_per_slide=counts,
