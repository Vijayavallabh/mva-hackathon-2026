#!/usr/bin/env python3
"""Editorial v16 release over falsification v15; preserve all earlier bound artifacts."""
import argparse
...
import re
import shutil
import track2_release_v15 as historical
...
def current_path(value):
    for name in ['report','slides','pitch','transcript','video-description']:
        value=value.replace(f'track2-{name}-v15',f'track2-{name}-v16')
    return value.replace('track2-v15-design','track2-v16-design').replace('track2-v15-render-audit','track2-v16-render-audit')

FILES={name.replace('v15','v16') if current_path(source)!=source else name:current_path(source)
       for name,source in historical.FILES.items()}
INPUTS=historical.INPUTS | set(FILES.values()) | {
    'notes/track2-v16-design.md','scripts/track2_release_v16.py',
    'scripts/render_track2_slides_v16.mjs','scripts/test_track2_release_v16.py'}
FIELDS={'schema_version','created_utc','files','input_hashes','historical_v15_manifest',
...
def presentation_checks(deck):
    parser=historical.historical.DeckParser();parser.feed(deck);parser.close();parser.validate()
    ...
    for path in ['track2-report-v16.md','alphafold3-output-terms.md','alphafold3-Legally-Binding-Terms-of-Use.txt']:
...
def check_deck():
    path=ROOT/'notes/track2-slides-v16.html'
...
def report_checks(report):
    require('[v15 decision ledger]' in report and 'Its 63 cumulative source records' in report,'Current ledger route drift')
    abstract=report.split('**A17 — Method abstract (under 500 words).**')[1].split('## Acknowledgement')[0]
    require(len(abstract.split())<500,'Method abstract over limit')
    for phrase in ['8/12','11/24','post-hoc sensitivity','1,000-fold','unknown/hold','measurement artifact']:
        require(phrase in abstract,'Method abstract omits falsification limit: '+phrase)
    require('strengthen the computational reason' not in ' '.join(abstract.split()),'Unqualified candidate-score inference')
...
def evidence_checks():
    ...
    model=historical.historical.model_evidence_checks()
    report=(ROOT/'notes/track2-report-v16.md').read_text()
    ...
    for filename in ['track2-report-v16.md','track2-video-description-v16.md']:
...
def scientific_checks():
    ...
    pitch=(ROOT/'notes/track2-pitch-v16.md').read_text()
    ...
    transcript='Track 2 v16 — read-aloud transcript\nRead the paragraphs; headings and times are cues, not narration.\n\n'
    for header,paragraph in paragraphs:
        transcript+='Slide '+header.replace(' / ',' | ')+'\n'+paragraph.strip()+'\n\n'
    require((ROOT/'notes/track2-transcript-v16.txt').read_text()==transcript,'Plain transcript drift')
    review=json.loads((ROOT/'notes/track2-v16-render-audit.json').read_text())
    require(review['source_sha256']==deck['source_sha256'],'Audited deck hash drift')
    require(review['renderer_sha256']==digest(ROOT/'scripts/render_track2_slides_v16.mjs'),'Renderer hash drift')
...
def history():
    p=ROOT/'results/feat009/jvv7_track2_research_v15'
...
def location(path):
    ...
    require(path.name!='jvv7_track2_research_v15','Historical v15 protected')
    return path
...
def build(path):
    ...
    manifest=dict(schema_version=16,created_utc=datetime.now(timezone.utc).isoformat(),
        files={name:digest(path/name) for name in FILES},input_hashes=bound,historical_v15_manifest=old,
...
def verify(path):
    ...
    require(m['schema_version']==16 and m['input_hashes']==inputs(),'Input/schema drift')
    ...
    require(m['historical_v15_manifest']==history(),'Historical drift')
    ...
    return dict(integrity_verified=True,files=len(FILES),bound_inputs=len(INPUTS),historical_v1_through_v15_preserved=True,upload_ready=False)
